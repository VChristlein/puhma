...
int CommandOverlaySegmentation::execute()
{
	...
	config.annotation_color[comma1] = ' ';
	config.annotation_color[comma2] = ' ';
	cv::Vec3d annot;
    // if everything else is ok, start copying stuff
	{
		std::stringstream s;
		s << config.annotation_color;
		s >> annot[2] >> annot[1] >> annot[0];
/*
...
*/
	...
	if (config.verbosity > 0) {
		std::cout << "annotation color (normalized between 0 and 1): " << annot[2] << "," << annot[1] << "," << annot[0] << std::endl;
	}
