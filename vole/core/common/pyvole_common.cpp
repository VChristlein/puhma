...
static PyObject * convert(cv::Mat mat){
        ...
        Py_ssize_t *shape = new Py_ssize_t[nd];
        shape[0]=mat.rows; shape[1]=mat.cols;
        if (nd==3) shape[2]=mat.channels(); // war vorher 3 
        // get Mat type
        int np_type;
...
struct Mat_from_python_ndarray{
    ...
    */
    static void* convertible(PyObject* obj_ptr){
        ...
        if (PyArray_NDIM(obj_ptr)<2) {
          std::cerr << "Mat_from_python_ndarray dimension error: PyArray_NDIM(obj_ptr)<2" << std::endl;
          return NULL;
        }
        return obj_ptr;
    }
    ...
    */
    static void construct(PyObject* obj_ptr,
        ...
        // Construct cv::Mat
//        std::cerr << itemsize << "  " << cv_depth << "  " << size[0] << "  " << size[1] << "  " <<steps << std::endl;
...
public:
    ...
    */
    static void* convertible(PyObject* obj_ptr){
//        std::cerr << "testing convertability for Mat_T" << std::endl;
        if (!PyArray_Check(obj_ptr)) return NULL;
...
void initializeConverters() {
    ...
    // vector converters
    #define VECTOR_CONV(Type) custom_vector_from_seq<Type>();  boost::python::to_python_converter<std::vector<Type>, custom_vector_to_list<Type> >();
    VECTOR_CONV(bool);
    VECTOR_CONV(char);
    VECTOR_CONV(int);
    VECTOR_CONV(float);
    VECTOR_CONV(double);
    VECTOR_CONV(cv::Mat);
    VECTOR_CONV(cv::Mat1b);
    VECTOR_CONV(cv::Mat1f);
    VECTOR_CONV(cv::Mat1d);
}

void printInfo(){
    std::cout<<"Boost Python converters for opencv-numpy"<<std::endl;
}
