...
FisherV :: FisherV(const AttriConfig &c, const FileHandler *fpm, Statistic *const st)
...
void FisherV :: compute(const std::vector<std::vector<std::string> > &data,
                   const cv::PCA pca,
                   const cv::Mat &mGMM,
...
cv::Mat FisherV::computeOneFV(const cv::Mat &im,
                         const cv::PCA pca,
                         const cv::Mat &mGMM,
                         const std::vector<cv::Mat> &covsGMM,
                         const cv::Mat &weightsGMM,
...
cv::Mat FisherV :: fetchOneFeature(const std::string fn) const
{
    cv::Mat re;
    QString filep = QString::fromStdString(fn)+".yaml";
    QFileInfo info(QDir(QString::fromStdString(filePathManager->featsDir)), filep);
    if( !info.exists() )
        qDebug() << "WARNING: file doesn't exist:" << info.filePath() << "from" << QString::fromStdString(fn);
    ...
    //qDebug() << re.rows << re.cols << re.type() << re.channels();
    return re;
}
