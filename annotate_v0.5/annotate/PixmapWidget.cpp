...
PixmapWidget::PixmapWidget(QAbstractScrollArea* parentScrollArea,
                           QWidget *parent)
	: QWidget(parent)
{    
	...
	setMinimumSize(static_cast<int>(m_pm->width()*zoomFactor),
				   static_cast<int>(m_pm->height()*zoomFactor));
...
void PixmapWidget::setZoomFactor( double f )
{
    ...
    int w = static_cast<int>(m_pm->width()*zoomFactor);
    int h = static_cast<int>(m_pm->height()*zoomFactor);
	setMinimumSize( w, h );
